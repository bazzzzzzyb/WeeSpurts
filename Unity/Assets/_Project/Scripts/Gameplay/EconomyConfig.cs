...
namespace WeeSpurts.Gameplay
{
    ...
    /// <see cref="DEFAULT_STARTING_TICKETS"/> if no asset is assigned, so a scene
    ...
    public class EconomyConfig : ScriptableObject
    {
        ...
        public const int DEFAULT_STARTING_TICKETS = 500;

        [Tooltip("Tickets each player starts a SESSION with (not each match — the ledger lives on GameManager and survives scene loads, so winnings carry between games). Placeholder: big enough to afford a few rounds at the bar and a couple of hands of blackjack without thinking about it, which is what you want while judging whether the casino corner is fun at all.")]
        public int StartingTickets = DEFAULT_STARTING_TICKETS;
    }
}
