...
1. 🧑 **Install Mirror**: GitHub, not the Asset Store — the Asset Store copy lags and this project (Unity 6000.5.4f1) needs the newest build. github.com/MirrorNetworking/Mirror → Releases → latest `.unitypackage` (confirmed via the mirror-kcp spike: **v96.11.1**) → in Unity: Assets → Import Package → Custom Package. Commit the imported `Assets/Mirror/` folder to the branch (it's source the project needs to compile, not a rebuildable cache); if `.dll`s turn up under it (Mono.Cecil, BouncyCastle — they did in the spike), add `*.dll` to `.gitattributes`' Git LFS rules before committing. Full reasoning: `Docs/spikes/2026-08-03-mirror-kcp-findings.md`.
